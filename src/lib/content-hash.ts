...
export function hashContent(text: string): string {
	...
	return h.map((value) => value.toString(16).padStart(8, '0')).join('').slice(0, 16);
}

export function parseFrontmatter(text: string): { hash: string | null; body: string } {
	const match = text.match(/^---\r?\n([\s\S]*?)\r?\n---\r?\n([\s\S]*)$/);
	if (!match) return { hash: null, body: text };

	const hash = match[1].match(/hash:\s*(\S+)/)?.[1] ?? null;
	return { hash, body: match[2].trimStart() };
}
